 ...
 * @param {boolean=} options.ensureRelationsVisibleAndInvisible A boolean which specifies
 * whether the plugin should load all relations listed in an `ensureRelationsLoaded` array,
 * regardless of whether they will be visible properties. Default plugin behavior
 * corresponds to a value of `false`, which avoids the work of loading relations
 * that will not be present in the serialization result. `true` might
 * be useful if you have provided `options.handleEnsureRelation`.
...
module.exports = function(options) {
  ...
  return function(Bookshelf) {
    ...
    var Model = Bookshelf.Model.extend({
      ...
      /**
       * @desc Sets the accessor on the model. An alternative to setting the
                ...
                if (tableContextEnsureRelations) {
                    ...
                    relationNamesToEnsurePromise = contextDesignationPromise
                      .then(function(contextDesignation) {
                        ...
                        if (!Array.isArray(relationNames)) {
                          throw new SanityError('evaluator function did not successfully ' +
                            'identify relation names to ensure loaded');
                        }
                  ...
                  ensureRelationsPromise = relationNamesToEnsurePromise.bind(this)
                    .then(function(relationNamesToEnsure) {
                      ...
                      var loadTheseRelations;
                      if (ensureRelationsVisibleAndInvisible) {

                        loadTheseRelations = relationNamesToEnsure;

                      } else {
                        // Load only those relations that are also in ultimatelyVisibleFields,
                        // to avoid unnecessary work.

                        var visibleRelationNamesToEnsure = _.intersection(ultimatelyVisibleFields,
                          relationNamesToEnsure);

                        if (visibleRelationNamesToEnsure.length !== relationNamesToEnsure.length) {
                          console.log('bookshelf-advanced-serialization plugin: You have ' +
                            'specified relation names in `ensureRelationsLoaded` ' +
                            'which are not visible properties of the model. Not loading ' +
                            'such relations. If you want to force their loading, use the ' +
                            '`ensureRelationsVisibleAndInvisible` option.'
                          )
                        }

                        loadTheseRelations = visibleRelationNamesToEnsure;
                      }

                      return BluebirdPromise.map(loadTheseRelations,
