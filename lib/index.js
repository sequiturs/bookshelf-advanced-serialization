...
module.exports = function(options) {
  ...
  return function(Bookshelf) {
    ...
    var Model = Bookshelf.Model.extend({
       ...
       *
       * Note that this method mutates models, by loading relations per
       * `options.ensureRelationsLoaded` and by removing relations that are not
       * among visible properties (the latter improves serialization performance
       * and helps to avoid infinite-looping / cycling of serialization).
       * Such mutating behavior is, admittedly, not obvious from the method's
       * name of `toJSON` alone. In using this plugin, you should think of
       * `toJSON` not as merely converting the existing model to a serialized
       * form, but as *transforming* it according to the  user's specifications,
       ...
       * with values that are an array of visible property names.
